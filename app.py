...
# ==============================================================================
@st.cache_resource(show_spinner="Loading Real-IAD PatchCore model artifact...")
def load_model(model_path="realiad_patchcore_model.pt"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    if not os.path.exists(model_path):
        return None, None, None, None, None, device, f"Model file '{model_path}' not found. Please train and save the model first."
    ...
    feature_extractor.load_state_dict(artifact["backbone_state_dict"])
    ...
    # Read configuration from artifact
    config = {
        "dataset": artifact.get("dataset", "Real-IAD"),
        "backbone": artifact.get("backbone", "Wide ResNet-50-2"),
        "feature_layers": artifact.get("feature_layers", ["layer2", "layer3"]),
        ...
        "distance_metric": artifact.get("distance_metric", "euclidean"),
        "normalize_mean": artifact.get("normalize_mean", [0.485, 0.456, 0.406]),
        "normalize_std": artifact.get("normalize_std", [0.229, 0.224, 0.225]),
        "gaussian_kernel": artifact.get("gaussian_kernel", 33),
        "gaussian_sigma": artifact.get("gaussian_sigma", 4.0),
    }
    
    # Construct transform once from artifact configuration
    transform = T.Compose([
    ...
    return feature_extractor, memory_banks, thresholds, transform, config, device, None
...
def predict(image: Image.Image, category: str, feature_extractor, memory_banks, thresholds, transform, config, device):
    ...
    mem_bank = memory_banks[category].to(device)
    threshold = float(thresholds[category])
    
    with torch.no_grad():
        feats = feature_extractor(input_tensor)  # [1, 1536, 64, 64]
        B, C, H, W = feats.shape
    ...
    # Generate 0.5 * orig + 0.5 * heatmap overlay
    ...
    heatmap_rgb = cm.jet(norm_map)[..., :3]
    overlay_np = np.clip(0.5 * orig_np + 0.5 * heatmap_rgb, 0.0, 1.0)
...
def main():
    ...
    st.markdown('<div class="sub-title">PatchCore-style Anomaly Detection</div>', unsafe_allow_html=True)
    ...
    if uploaded_file is not None:
        ...
        if "res" in st.session_state:
            ...
            with col_img2:
                ...
                im = ax.imshow(res["anomaly_map"], cmap="jet")
                ax.axis("off")
